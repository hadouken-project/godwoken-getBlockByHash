...
async function runTx(opts) {
    ...
    opts.block = opts.block ?? ethereumjs_block_1.Block.fromBlockData({}, { common: opts.tx.common });
    ...
    if (this._common.isActivatedEIP(2929) === true) {
        // console.log("before clearWarmedAccounts")
        state.clearWarmedAccounts();
        // console.log("after clearWarmedAccounts")
    }
    ...
    try {
        // console.log("before _runTx.bind")
        const result = await _runTx.bind(this)(opts);
        // console.log("before state.commit")
        await state.commit();
        if (this.DEBUG) {
...
async function _runTx(opts) {
    ...
    // console.log("before getSenderAddress");
    ...
    if (this._common.isActivatedEIP(3607) === true && !fromAccount.codeHash.equals(ethereumjs_util_1.KECCAK256_NULL)) {
        const msg = _errorMsg('invalid sender address, address is not EOA (EIP-3607)', this, block, tx);
        throw new Error(msg);
    }
    // console.log("before getUpfrontCost");
    ...
    // console.log("after getUpfrontCost");
    ...
    // console.log("before tx.supports 2");
    // EIP-1559 tx
    if (tx.supports(ethereumjs_tx_1.Capability.EIP1559FeeMarket)) {
        const baseFee = block.header.baseFeePerGas;
        // console.log("before inclusionFeePerGas if");
        inclusionFeePerGas =
            tx.maxPriorityFeePerGas <
                tx.maxFeePerGas - baseFee
                ? tx.maxPriorityFeePerGas
                : tx.maxFeePerGas - baseFee;
        // console.log("before gasPrice");
        gasPrice = inclusionFeePerGas + baseFee;
        // console.log("after gasPrice");
    }
    else {
    ...
    // console.log('data: ', (0, ethereumjs_util_1.short)(data))
    // console.log('to: ', to)
    if (this.DEBUG) {
        debug(`Running tx=0x${tx.isSigned() ? tx.hash().toString('hex') : 'unsigned'} with caller=${caller} gasLimit=${gasLimit} to=${to?.toString() ?? 'none'} value=${value} data=0x${(0, ethereumjs_util_1.short)(data)}`);
    }
    // console.log("before this.evm.runCall");
    ...
    if (this.DEBUG) {
        ...
        debug('-'.repeat(100));
        debug(`Received tx execResult: [ executionGasUsed=${executionGasUsed} exceptionError=${exceptionError ? `'${exceptionError.error}'` : 'none'} returnValue=0x${(0, ethereumjs_util_1.short)(returnValue)} gasRefund=${results.gasRefund ?? 0} ]`);
    }
    /*
     * Parse results
     */
    ...
    let miner;
    // console.log("before consensusType", this._common.consensusType())
    if (this._common.consensusType() === ethereumjs_common_1.ConsensusType.ProofOfAuthority) {
        // console.log("cliqueSigner", block.header.cliqueSigner())
    ...
    // console.log("before state.cleanupTouchedAccounts");
    await state.cleanupTouchedAccounts();
    // console.log("before state.clearOriginalStorageCache");
...
async function generateTxReceipt(tx, txResult, cumulativeGasUsed) {
    ...
    if (!tx.supports(ethereumjs_tx_1.Capability.EIP2718TypedTransaction)) {
        // Legacy transaction
        if (this._common.gteHardfork(ethereumjs_common_1.Hardfork.Byzantium) === true) {
            // Post-Byzantium
            receipt = {
